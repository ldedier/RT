...
static t_point3d	screen2world(t_pixel pix, t_world *world)
{
	...
	scale.z = cam->fd;
	p = move(move(move(cam->o, cam->look, scale.z),
...
static int		shadowhit(t_world *world, t_hit *hit, t_line *sray)
{
	...
	sray->o = move(hit->point, sray->v, EPSILON);
...
t_color				render_pixel(t_world *world, t_pixel pix)
{
	...
	if ((hit = trace(line, world->objlist)))
	{
		if (shadowhit(world, hit, &sray))
			return (freeret(
interpole_color(AMBIENT_LIGHT, BLACK_COLOR, hit->obj.c), &hit));
			illu = dotprod(hit->obj.normal_func(hit->obj, hit->point),
					sray.v) * world->lights[0].intensity;
