...
/*   Updated: 2018/05/21 08:13:11 by lcavalle         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_point3d	pert_plane(t_hit *hit)
{
	...
	yaxis = (t_point3d){.x = 0, .y = -1, .z = 0};
	//plane.ptbn.v = (t_point3d){.x=rand() % 100 / 50.0 - 1.0,.y=0,.z=rand() % 100 / 50.0 - 1.0};
	hit->obj.ptbn.v = crossprod(hit->point, yaxis);
	double angle = sin(magnitude(hit->point));
	pert = rotate_axis(hit->normal, hit->obj.ptbn.v, angle);
