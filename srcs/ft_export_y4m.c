...
/*   Updated: 2018/06/11 08:26:57 by lcavalle         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "rt.h"

void	ft_print_pixel_rgb_to_y_cb_cr(int pixel, int fd, int print)
{
	t_color_l			color;
	t_rgb_to_y_cb_cr	t;
	int					y;
...
void	ft_print_frame(int *frame, int length, int fd)
{
	...
	i = -1;
	while (++i < length)
		ft_print_pixel_rgb_to_y_cb_cr(frame[i], fd, 1);
	i = -1;
	while (++i < length)
		ft_print_pixel_rgb_to_y_cb_cr(frame[i], fd, 2);
	i = -1;
	while (++i < length)
		ft_print_pixel_rgb_to_y_cb_cr(frame[i], fd, 3);
}
...
void	ft_print_frames(t_video *video, t_list **list, int fd)
{
	...
	{
		ft_dprintf(fd, "FRAME\n");
		ft_print_frame((int *)((*list)->content), video->width *
				video->height, fd);
		tmp = *list;
		*list = (*list)->next;
		free(tmp->content);
