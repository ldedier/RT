...
/*   Updated: 2018/06/03 07:44:09 by ldedier          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		texture_sphere(t_object obj, t_hit *hit)
{
	...
	float u = (int)(ft_dot_product(hit->old_point, m_UAxis) * obj.parser.width) % (obj.parser.width);
	float v = (int)(ft_dot_product(hit->old_point, m_VAxis) * obj.parser.height) % (obj.parser.height);
	if (u < 0)
		u *= -1;
	if (v < 0)
		v *= -1;
