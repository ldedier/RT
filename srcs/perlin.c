...
/*   Updated: 2018/06/12 14:35:00 by lcavalle         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	aux_perlin(t_auxperlin *aux, t_point3d *xyz, int *p)
{
...
static void	fill_p(int *p, int *permutation)
{
	int			i;
...
double		perlin(double x, double y, double z)
{
	...
	xyz = (t_point3d){.x = x, .y = y, .z = z};
	permutation = get_permutation();
	fill_p(p, permutation);
	aux_perlin(&aux, &xyz, p);
	return (lerp(aux.fz, lerp(aux.fy,
					lerp(aux.fx, grad(p[aux.aa], xyz.x, xyz.y, xyz.z),
						grad(p[aux.ba], xyz.x - 1, xyz.y, xyz.z)),
					lerp(aux.fx, grad(p[aux.ab], xyz.x, xyz.y - 1, xyz.z),
						grad(p[aux.bb], xyz.x - 1, xyz.y - 1, xyz.z))),
				lerp(aux.fy, lerp(aux.fx,
