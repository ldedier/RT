...
/*   Updated: 2018/06/12 01:30:04 by aherriau         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "rt.h"

void	ft_menu_objects_bloc2_rangebars_2(t_world *world, int i, int pos)
{
	if (pos >= world->menu.objects_rb[i].pix.x &&
			pos <= (world->menu.objects_rb[i].pix.x + 100))
	{
...
void	ft_menu_objects_bloc2_rangebars(t_world *world)
{
	...
	while (i < world->menu.nb_objects_rb)
	{
		...
		ft_add_menu_surface(world);
		pos = world->menu.objects_rb[i].pix.x
			+ ((*(world->menu.objects_rb[i].value) * 100)
					/ world->menu.objects_rb[i].max);
		ft_menu_objects_bloc2_rangebars_2(world, i, pos);
		i++;
	}
}
