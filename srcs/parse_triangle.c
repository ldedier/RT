...
void	ft_process_parsing_vertex_a(t_parser *parser, t_world *world, char *line)
{
	t_point3d	*a;

	if (parser->parse_enum == e_parse_object)
		a = &(world->cobjlist->cobject->objlist->object->object_union.triangle.v1);
	else
	{
...
void	ft_process_parsing_vertex_b(t_parser *parser, t_world *world, char *line)
{
	...
	if (parser->parse_enum == e_parse_object)
		b = &(world->cobjlist->cobject->objlist->object->object_union.triangle.v2);
	else
	{
		ft_dprintf(2, "line %d: current object can not have a vertex tag\n",
				parser->nb_lines);
...
void	ft_process_parsing_vertex_c(t_parser *parser, t_world *world, char *line)
{
	...
	if (parser->parse_enum == e_parse_object)
		c = &(world->cobjlist->cobject->objlist->object->object_union.triangle.v3);
	else
	{
		ft_dprintf(2, "line %d: current object can not have a vertex tag\n",
				parser->nb_lines);
